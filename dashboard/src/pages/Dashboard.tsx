...
import { CheckCircle2, XCircle, Clock, Search, Server, Activity, ArrowUpRight } from 'lucide-react';
...
function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center">
            ...
            <div className="mockup-code bg-base-300 text-base-content text-left shadow-none border border-base-content/10">
              <pre data-prefix="$">
                <code>alloy run -c "xcodebuild test -scheme MyApp"</code>
              </pre>
            </div>
          </div>
        ) : (
            ...
            <table className="table table-lg w-full">
              <thead className="bg-base-200/50">
                <tr>
                  ...
                  <th className="font-semibold text-xs tracking-wider opacity-70">
                    JOB ID / WORKFLOW
                  </th>
                  <th className="font-semibold text-xs tracking-wider opacity-70 hidden md:table-cell">
                    AGENT
                  </th>
                  <th className="font-semibold text-xs tracking-wider opacity-70 hidden md:table-cell">
                    DURATION
              ...
              <tbody>
                {filteredJobs.map((job) => (
                  ...
                  >
                    <td>
                      <div
                        className={`badge gap-2 h-8 pl-2 pr-3 ${
                          job.status === 'running'
                      ...
                      <div>
                        <div className="flex items-baseline gap-2">
                          ...
                          <span className="font-bold">
                            {(job.command || job.script || 'Build').split(' ')[0]}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-base-content/60 mt-1">
                          <span className="font-mono bg-base-200/80 px-1 rounded border border-base-300">
                            {job.id.slice(-7)}
                          </span>
                          <span className="truncate max-w-[200px]">
                            {(job.command || job.script || '').substring(0, 30)}
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="font-mono text-sm hidden md:table-cell">
                      {formatDuration(job)}
                    </td>
                    <td>
                      <Link
      ...
      {/* Live Log Panel */}
      {selectedJob && <LiveLogPanel job={selectedJob} onClose={() => setSelectedJob(null)} />}
    </div>
  );
}
